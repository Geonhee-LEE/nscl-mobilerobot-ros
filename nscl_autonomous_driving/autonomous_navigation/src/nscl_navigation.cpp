...
#include "autonomous_navigation/nscl_navigation.h"


NSCLNavigation::NSCLNavigation()
{
	cout << "##### start NSCL Navigation ##### " <<endl;

	statePublisher = nh_.advertise<std_msgs::String>("/nscl_navigation/state", 1000);
    ...
    //setGoalSubscriber = nh_.subscribe("/nscl_navigation/set_goal", 1000, &NSCLNavigation::setGoal, this);
	stopSubscriber = nh_.subscribe("/nscl_navigation/stop", 1000, &NSCLNavigation::stop, this);
	globalPlanSubscriber = nh_.subscribe("/move_base/GlobalPlanner/plan", 1000, &NSCLNavigation::checkGlobalPlan, this);
	currentGoalSubscriber = nh_.subscribe("/move_base/current_goal", 1000, &NSCLNavigation::setCurrentGoal, this);
	currentPoseSubscriber = nh_.subscribe("/amcl_pose", 1000, &NSCLNavigation::setCurrentPose, this);
    ...
    sendgoal_flgs = false;
	cout << "##### nscl navigation connect finished #####" <<endl;
}
...
void NSCLNavigation::recoveryBehavior() 
{
    
	ros::Duration(2).sleep();
	ROS_INFO("nscl recoveryBehavior start");
	...
	if(comminfo_msg.sonar.c_str()[0] == '1' && comminfo_msg.sonar.c_str()[6] == '1')
	{
		...
		ROS_INFO("nscl recoveryBehavior rotation!!!!");
	}
	// Not detect
	else if(comminfo_msg.sonar.c_str()[0] == '0' && comminfo_msg.sonar.c_str()[6] == '0')
	{
		...
		ROS_INFO("nscl recoveryBehavior backward!!!!");		
	}
...
bool NSCLNavigation::setGoal(geometry_msgs::TransformStamped transformStamped) 
{
...
void NSCLNavigation::stop(actionlib_msgs::GoalID goal) 
{
	geometry_msgs::Twist cmd_vel;
	cmd_vel.linear.x = 0;
	cmd_vel.angular.z = 0;
...
void NSCLNavigation::teleop(geometry_msgs::Twist cmd_vel)
{
	cmdVelPublisher.publish(cmd_vel);
}

void NSCLNavigation::checkGlobalPlan(nav_msgs::Path path) 
{
...
float NSCLNavigation::calculateDistance(float x1, float y1, float x2, float y2) 
{
...
void NSCLNavigation::setCurrentGoal(geometry_msgs::PoseStamped goal) 
{
	oldPathPoseSize = 9999;
	currentGoal = goal;
}

void NSCLNavigation::setCurrentPose(geometry_msgs::PoseWithCovarianceStamped pose) {
	currentPose = pose;
}
