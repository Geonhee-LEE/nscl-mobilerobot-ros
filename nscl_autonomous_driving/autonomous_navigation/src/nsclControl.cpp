...
NSCLControl::NSCLControl()
{
	ROS_INFO("### NSCL control ###");	
  teleop_msg.data = false;
  cmd_pub_cnt = 0;
  led_pub_cnt = 0;
  
	cmd_srv = nh_.advertiseService("/nscl/command", &NSCLControl::cmdInfoCb, this);
  ...
  velSubscriber = nh_.subscribe("/cmd_vel", 1000, &NSCLControl::getVelCb, this);
  mapSubscriber = nh_.subscribe("/map_metadata", 1000, &NSCLControl::mapInfoCb, this);
...
void NSCLControl::doAutoNav()
{
  while(ros::ok())
  {
      ...
      {  
        ...
        ros::param::set("/nscl/status/response", os.str());   
      ...
      {
        execStop();
        ros::param::set("/nscl/status/response", "stop");  
      }
      ...
      {        
        ros::param::set("/nscl/status/response", this->current_cmdInfo.request.command);  
      }  
      else if(this->current_cmdInfo.request.command == "success" )
      {              
        ros::param::set("/nscl/status/response", "success"); 
      }
      else
      {
        execStop();
...
void NSCLControl::initPose(float x, float y, float theta)
{
...
void NSCLControl::getVelCb(geometry_msgs::Twist cmd_vel)
{
  current_vel = cmd_vel;
}

void NSCLControl::mapInfoCb(nav_msgs::MapMetaData map_data)
{
  g_map_data = map_data;
}

bool NSCLControl::cmdInfoCb(nscl_msgs::CmdInfo::Request  &req, nscl_msgs::CmdInfo::Response &res)
{
...
void NSCLControl::updataDynCfg( string str)
{
  if(str == "slow")
  {  
...
geometry_msgs::TransformStamped NSCLControl::calcPixel2Pos(float goal_x, float goal_y, float goal_theta)
{
  geometry_msgs::TransformStamped transformStamped;
...
geometry_msgs::Pose NSCLControl::getPixel2Pose(float goal_x, float goal_y, float goal_theta)
{
...
void NSCLControl::execMoveTo(geometry_msgs::TransformStamped goal_transformStamped)
{        
  ...
  if(msclnav.setGoal(goal_transformStamped) && msclnav.sendgoal_flgs == true)
  {
    this->current_cmdInfo.request.command = "success" ;
  }
  else if(msclnav.sendgoal_flgs == true)
  {          
...
void NSCLControl::execTeleop(float trans_vel, float angular_vel)
{
...
void NSCLControl::updataSlowMode( )
{
    updataSlowLocalMap();
    updataSlowDWAPlan();
}

void NSCLControl::updataSlowLocalMap( )
{
...
void NSCLControl::updataSlowDWAPlan( )
{
  std::vector<dynamic_reconfigure::IntParameter> vec_int;
  std::vector<dynamic_reconfigure::DoubleParameter> vec_db;
  std::vector<dynamic_reconfigure::StrParameter> vec_str;
...
void NSCLControl::updataNormalMode( )
{
    updataNormalLocalMap();
    updataNormalDWAPlan();

}
void NSCLControl::updataNormalLocalMap( )
{
  std::vector<dynamic_reconfigure::IntParameter> vec_int;
  std::vector<dynamic_reconfigure::StrParameter> vec_str;
  std::vector<dynamic_reconfigure::DoubleParameter> vec_db;
...
void NSCLControl::updataNormalDWAPlan( )
{
  std::vector<dynamic_reconfigure::IntParameter> vec_int;
  std::vector<dynamic_reconfigure::DoubleParameter> vec_db;
...
void NSCLControl::updataFastMode( )
{
    updataFastLocalMap();
    updataFastDWAPlan();

}
void NSCLControl::updataFastLocalMap( )
{
  std::vector<dynamic_reconfigure::IntParameter> vec_int;
  std::vector<dynamic_reconfigure::StrParameter> vec_str;
  std::vector<dynamic_reconfigure::DoubleParameter> vec_db;
...
void NSCLControl::updataFastDWAPlan()
{
  std::vector<dynamic_reconfigure::IntParameter> vec_int;
  std::vector<dynamic_reconfigure::DoubleParameter> vec_db;
